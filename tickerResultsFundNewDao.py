...
class TickerResultsFundNew(db.Model):
  ...
  rank=db.FloatProperty(default=0.0, required=False);


def TickerResultsFundNew_key(date):
#  """Constructs a datastore key for a Optimal Values entity with the date key."""
  return db.Key.from_path('TickerResultsFundNew', date)
...
def getAllTickerResultsFundNew(page, uri ):
    tickerResults=None;
    pageSize=getPageSize();

    myPagedQuery = PagedQuery(TickerResultsFundNew.all(), pageSize)
